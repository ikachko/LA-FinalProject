...
Non-negative Matrix Factorization (NMF) \cite{nmf} is another method often recommended to use to perform blind source separation. It factorize matrix \textbf{X} into matrix of basis vectors \textbf{W} and matrix of weights \textbf{H} with a property, that all three matrices have no negative elements. 

$$X = W H$$

Multiplication above can be interpreted as computing the column of $Xi$ as a linear combinations of columns vectors in $W$ using coefficients supplied by columns of $H$. Which are exactly audio sources and mixing matrix that we are aiming to restore.
